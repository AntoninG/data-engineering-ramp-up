...
import sqlalchemy
from apache_beam.io import ReadFromText, WriteToAvro, WriteToText
...
class WriteToCloudSQL(beam.DoFn):
    def process(self, element):
        ...
        insert_stmt = sqlalchemy.text(
            "INSERT INTO git_downloads_agg (os_name, os_version,no_f_downloads) VALUES (:os_name, :os_version,:no_f_downloads)",
        )
        ...
        with pool.connect() as db_conn:
            ...
            create_table = sqlalchemy.text(
                "CREATE TABLE IF NOT EXISTS git_downloads_agg(os_name VARCHAR(20), os_version VARCHAR(20), no_f_downloads INT)"
            )
            ...
            db_conn.execute(
                ...
                parameters={
                    ...
                    "no_f_downloads": int(element["no_f_downloads"]),
                },
            )
            db_conn.commit()
...
def run():
    ...
    with beam.Pipeline(options=beam_options) as pipeline:
        ...
        read_raw = pipeline | "Read" >> beam.io.ReadFromAvro(
            "gs://dataflow_demos2/input_data/pypy_filedownloads.avro"
        )

        # Filter , Clean and Aggregate data ,Number of PYPY downloads by country,project, version
        agg_cntry = (
            read_raw
            | "Filter Data" >> beam.Filter(lambda x: x["details"]["python"].startswith("3.10"))
            | "Get required data"
            >> beam.Map(
                lambda x: (
                    x["country_code"] + "," + x["project"] + "," + x["details"]["python"],
                    x["timestamp"],
                )
            )
            | "Combine per key" >> beam.combiners.Count.PerKey()
            | "Make it to dict again"
            >> beam.Map(
                lambda x: {
                    "country_code": x[0].split(",")[0],
                    "project": x[0].split(",")[1],
                    "python_version": x[0].split(",")[2],
                    "no_of_downloads": x[1],
                }
            )
        ...
        agg_cntry | "WriteToAvro" >> WriteToAvro(
            "gs://dataflow_demos2/output_data/agg_output_data.avro",
            schema={
                "type": "record",
                "name": "agg_downloads",
                "fields": [
        ...
        aggr_os_version = (
            ...
            | "Filter Data with OS"
            >> beam.Filter(
                lambda x: x["details"]["system"]["name"] == "Windows"
                and x["details"]["rustc_version"] != None
            )
            | "Get os data woth others"
            >> beam.Map(
                lambda x: (
                    x["details"]["system"]["name"] + "," + x["details"]["rustc_version"],
                    x["timestamp"],
                )
            )
            | "Combine per key os" >> beam.combiners.Count.PerKey()
            | "Make it dict of agg results"
            >> beam.Map(
                lambda x: {
                    "os_name": x[0].split(",")[0],
                    "os_version": x[0].split(",")[1],
                    "no_f_downloads": x[1],
                }
            )
            # | 'Print' >> beam.Map(print)
